...
def print_install_instructions(vsix_path: str) -> None:
  ...
  {opt}After installing:{rst}
    ...
    - Open a Flutter project with a pubspec.yaml
    - The Saropa Package Vibrancy sidebar shows dependency health
    - Press {key}Ctrl+Shift+P{rst} and type {key}"Saropa"{rst} to see all commands
""")


def prompt_install(vsix_path: str) -> None:
    """Ask whether to install the .vsix via the code CLI."""
    ...
    if result.returncode != 0:
        fail(f"Install failed: {(result.stderr or '').strip()}")
        return
