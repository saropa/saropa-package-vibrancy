...
describe('save-task-runner', () => {
    ...
    describe('SaveTaskRunner', () => {
        ...
        it('should respect onSaveChanges setting', () => {
            setTestConfig('saropaPackageVibrancy', 'onSaveChanges', 'flutter pub get');
            const runner = new SaveTaskRunner();
            runner.dispose();
        });

        it('should respect onSaveChangesDetection setting', () => {
            setTestConfig('saropaPackageVibrancy', 'onSaveChangesDetection', 'any');
            const runner = new SaveTaskRunner();
            runner.dispose();
        });
    ...
    describe('configuration validation', () => {
        ...
        it('should accept task: prefix format', () => {
            setTestConfig('saropaPackageVibrancy', 'onSaveChanges', 'task:Build');
            const config = vscode.workspace.getConfiguration('saropaPackageVibrancy');
            assert.strictEqual(config.get('onSaveChanges'), 'task:Build');
        });

        it('should accept empty string (disabled)', () => {
            setTestConfig('saropaPackageVibrancy', 'onSaveChanges', '');
            const config = vscode.workspace.getConfiguration('saropaPackageVibrancy');
